import pytest
import sys
# <--- Change the name projectoFP to the file name with your project
...
class TestPublicPosicao:
    ...
    def test_2(self):
        assert not fp.posicoes_iguais(
            fp.cria_posicao('a', 2), fp.cria_posicao('b', 3))

    def test_3(self):
        assert fp.posicoes_iguais(fp.cria_posicao(
            'a', 2), fp.str_para_posicao('a2'))
    ...
    def test_5(self):
        ...
        assert ('a1', 'b2', 'a3') == tuple(fp.posicao_para_str(i)
                                           for i in fp.obtem_posicoes_adjacentes(i1, 2, False))

    def test_6(self):
        i1 = fp.cria_posicao('a', 2)
        assert ('a1', 'b1', 'b2', 'b3', 'a3') == tuple(fp.posicao_para_str(i)
                                                       for i in fp.obtem_posicoes_adjacentes(i1, 2, True))

    def test_7(self):
        tup = (fp.cria_posicao('a', 1), fp.cria_posicao('a', 3),
               fp.cria_posicao('b', 1), fp.cria_posicao('b', 2))
        assert ('b2', 'a1', 'b1', 'a3') == tuple(fp.posicao_para_str(i)
                                                 for i in fp.ordena_posicoes(tup, 2))
...
class TestPublicPedra:
    ...
    def test_2(self):
        assert not fp.pedras_iguais(
            fp.cria_pedra_branca(), fp.cria_pedra_preta())
...
04 [X]-[ ]-[ ]-[O]"""
    ...
    def test_5(self):
        ...
        assert tuple((fp.posicao_para_str(p), fp.pedra_para_str(v))
                     for p, v in fp.obtem_linha_vertical(tab, pos)) == ref
    ...
    def test_6(self):
        ...
        assert tuple((fp.posicao_para_str(p), fp.pedra_para_str(v)) for p, v in diag) == ref_d and \
            tuple((fp.posicao_para_str(p), fp.pedra_para_str(v))
                  for p, v in anti) == ref_a
...
04 [X]-[X]-[ ]-[ ]"""
    ...
    def test_8(self):
        ...
        assert (not fp.verifica_linha_pedras(tab, fp.cria_posicao('d', 1), fp.cria_pedra_branca(), 3)) and \
            fp.verifica_linha_pedras(tab, fp.cria_posicao(
                'd', 2), fp.cria_pedra_branca(), 3)
    ...
    def test_11(self):
        ...
        assert tuple((fp.posicao_para_str(p), fp.pedra_para_str(v))
                     for p, v in fp.obtem_linha_horizontal(tab, pos)) == ref
    ...
    def test_12(self):
        ...
        assert tuple(fp.posicao_para_str(pos)
                     for pos in fp.obtem_posicoes_pedra(tab, p)) == ref


class TestPublicAdditionalFuns:
    def test_1(self):
        ib = tuple(fp.str_para_posicao(i)
                   for i in ('c1', 'c2', 'd2', 'd3', 'd4'))
        ip = tuple(fp.str_para_posicao(i)
                   for i in ('a3', 'a4', 'b1', 'b3', 'c3'))
        t = fp.cria_tabuleiro(2, ip, ib)
        assert (fp.eh_vencedor(t, fp.cria_pedra_preta()), fp.eh_vencedor(
            t, fp.cria_pedra_branca())) == (False, False)
    ...
    def test_2(self):
        ...
        assert (fp.eh_vencedor(t, fp.cria_pedra_preta()), fp.eh_vencedor(
            t, fp.cria_pedra_branca())) == (False, True)
    ...
    def test_6(self):
        ...
        ip = tuple(fp.str_para_posicao(i)
                   for i in ('a3', 'a4', 'b1', 'b3', 'c3'))
        t = fp.cria_tabuleiro(2, ip, ib)
        hyp_move, hyp_text = escolhe_movimento_manual_offline(
            t, 'c1\nc3\nc4\n')
