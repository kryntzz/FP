...
def obtem_posicoes_adjacentes(tab, pos):
    ...
    res, pos_c, coluna, linha, diagonais = (), 0, obtem_coluna(
        tab, pos), obtem_linha(tab, pos), obtem_diagonais(tab, pos)
...
def verifica_k_linhas(tab, pos, jg, k):
    ...
    if not pos in obtem_posicoes_jogador(tab, jg):
        return False
    t, c = (obtem_linha(tab, pos), obtem_coluna(tab, pos),
            obtem_diagonais(tab, pos)[0], obtem_diagonais(tab, pos)[1]), 0
    for i in t:
        pos_r = False
        for valor in i:
            if valor == pos:
    ...
    if len(res) > 0:
        ...
        for i in ordena_posicoes_tabuleiro(tab, obtem_posicoes_livres(tab)):
            ...
            while not eh_fim_jogo(tab_nv, k):
                ...
                tab_nv = marca_posicao_aux(
                    tab_nv, modo_normal(tab_nv, jg_o, k,), jg_o)
